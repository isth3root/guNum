...
const LeaderBoard = () => {
  ...
  const getScoreByDifficulty = (user: any) => {
    return difficulty === "EASY" ? user.score.easy : difficulty === "MEDIUM" ? user.score.medium : user.score.hard;
  };

  const sortedUsers = users.sort((a, b) => getScoreByDifficulty(a) - getScoreByDifficulty(b));

  return (
    <div className={`min-h-screen font-Teko flex flex-col ${themes === 'PINK' ? 'bg-[#FFEFEF] text-black' : ''} 
      ...
      <Link to={"/"} className="text-center underline text-4xl">Let's Play</Link>
      <div className="">
        <div className="mt-10">
          ...
          <ul className="flex flex-col">
            {sortedUsers.map((userItem, index) => (
              <li key={userItem.username} className={`text-5xl border-b-2 py-2 flex gap-5 w-full justify-around ${user?.username === userItem.username ? 'text-red-500' : ''}
